...
if [[ "X${out}" == "X" ]]; then
	curl -s ${cb_host}:8093/query/service -H 'Content-Type: application/json' -d "{\"statement\":\"SELECT ${collection}.* FROM ${bucket}._default.${collection} WHERE type='MD' AND docType != 'station' AND ;\"}" -u 'avid:pwd_av!d' | jq .results
else

...
function usage {
  echo "Usage $0 -c credentials-file [-o output_file]"
...
if [[ "X${out}" == "X" ]]; then
	curl -s ${cb_host}:8093/query/service -H 'Content-Type: application/json' -d "{\"statement\":\"SELECT ${collection}.* FROM ${bucket}._default.${collection} WHERE type='MD' AND docType != 'station' AND generated IS MISSING;\"}" -u 'avid:pwd_av!d' | jq .results
else
	curl -s ${cb_host}:8093/query/service -H 'Content-Type: application/json' -d "{\"statement\":\"SELECT ${collection}.* FROM ${bucket}._default.${collection} WHERE type='MD' AND docType != 'station' AND generated IS MISSING;\"}" -u 'avid:pwd_av!d' | jq .results > ${out}
fi
